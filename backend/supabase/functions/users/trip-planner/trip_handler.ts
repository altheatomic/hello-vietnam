...
export async function handleTripPlannerRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    switch (action) {
      case "planTrip":   return planTrip(userId, payload);
      case "getPlan":    return getPlan(userId, payload);
      case "listPlans":  return listPlans(userId);
      default:           return jsonResponse({ error: `Unsupported action: ${action}` }, 400);
    }
  } catch (err) {
...
async function planTrip(userId: string, p: JsonObject): Promise<Response> {
  const body = {
    id_user:     userId,
    id_province: reqStr(p.idProvince, "idProvince"),
