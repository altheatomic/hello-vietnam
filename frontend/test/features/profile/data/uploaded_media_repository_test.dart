...
void main() {
  ...
  test(
    'maps future AI media without fabricating it during an empty listing',
    () async {
      final UploadedMediaRepository repository = UploadedMediaRepositoryImpl(
        functionClient: SupabaseFunctionClient(
          accessTokenProvider: () async => 'test-token',
  ...
  test(
    'keeps an empty current listing empty without fabricating AI media',
    () async {
      final UploadedMediaRepository repository = UploadedMediaRepositoryImpl(
        functionClient: SupabaseFunctionClient(
          accessTokenProvider: () async => 'test-token',
          invoker:
              (
                ...
                Map<String, String>? headers,
                Object? body,
              }) async => <String, Object?>{'items': <Object?>[]},
        ),
      );
  ...
  test('skips malformed media rows while retaining valid rows', () async {
    ...
    expect(items.map((UploadedMediaItem item) => item.id), <String>[
      'valid-media',
    ]);
  });
      ...
      functionClient: SupabaseFunctionClient(
        ...
        invoker:
            (
              ...
              return <String, Object?>{
                'results': <Object?>[
                  <String, Object?>{'mediaId': 'media-1', 'status': 'deleted'},
                  <String, Object?>{
                    'mediaId': 'media-2',
                    'status': 'failed',
  ...
  test('skips delete results without a media id', () async {
        ...
        invoker:
            (
              ...
              'results': <Object?>[
                ...
                <String, Object?>{'mediaId': 'media-2', 'status': 'failed'},
              ],
            },
      ),
    );
    ...
    expect(summary.results.map((result) => result.mediaId), <String>[
      'media-2',
    ]);
    expect(summary.failedMediaIds, <String>['media-2']);
  });
  ...
  test('copies delete results so the summary cannot be mutated', () {
    ...
    final UploadedMediaDeleteSummary summary = UploadedMediaDeleteSummary(
      results,
    );
