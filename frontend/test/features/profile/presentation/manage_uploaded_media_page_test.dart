...
void main() {
  testWidgets(
    'lists forum media without trip choices and hides an empty AI group',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
  ...
  testWidgets(
    'opens a larger image preview popup without selecting the media',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
          _FakeUploadedMediaRepository(
            items: <UploadedMediaItem>[
      ...
      await tester.tap(
        find.byKey(const ValueKey<String>('media-image-forum-1')),
      );
      await tester.pumpAndSettle();

      expect(
        find.byKey(const ValueKey<String>('media-preview-dialog')),
        findsOneWidget,
      );
      expect(
        find.byKey(const ValueKey<String>('media-preview-image')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'selects one or all media before requiring irreversible confirmation',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
          _FakeUploadedMediaRepository(
            items: <UploadedMediaItem>[
      ...
      expect(
        find.byKey(const ValueKey<String>('media-page-title')),
        findsOneWidget,
      );
  ...
  testWidgets(
    'warns when selected media is the last image on an image-only post',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
          _FakeUploadedMediaRepository(
            items: <UploadedMediaItem>[
  ...
  testWidgets(
    'removes successfully deleted media and leaves the forum post untouched',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
          _FakeUploadedMediaRepository(
            items: <UploadedMediaItem>[
      ...
      expect(
        find.byKey(const ValueKey<String>('media-forum-2')),
        findsOneWidget,
      );
  ...
  testWidgets('shows an authentication or loading error state', (
    ...
    expect(
      find.text('Please sign in before using this feature.'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'reconciles an item marked failed when it is already gone on reload',
    (WidgetTester tester) async {
      final _FakeUploadedMediaRepository repository =
          _FakeUploadedMediaRepository(
            items: <UploadedMediaItem>[
  ...
  Future<UploadedMediaDeleteSummary> deleteMedia(
    ...
    if (removeItemsBeforeDeleteResult) {
      final Set<String> successfulIds = summary.results
          ...
          .toSet();
      items.removeWhere(
        (UploadedMediaItem item) => successfulIds.contains(item.id),
      );
    }
    return summary;
  }
