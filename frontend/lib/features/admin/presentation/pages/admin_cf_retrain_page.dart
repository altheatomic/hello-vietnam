...
const _cfBaseUrl = 'http://localhost:8000';
...
class _RetrainLog {
  ...
  factory _RetrainLog.fromJson(Map<String, dynamic> j) => _RetrainLog(
        idLog:       j['id_log']?.toString() ?? '',
        triggeredBy: j['triggered_by']?.toString() ?? '',
...
class AdminCfRetrainPage extends StatefulWidget {
  const AdminCfRetrainPage({super.key});
...
class _AdminCfRetrainPageState extends State<AdminCfRetrainPage> {
  ...
  Future<void> _fetchLogs() async {
    ...
    try {
      final res = await http.get(Uri.parse('$_cfBaseUrl/admin/cf/retrain/logs'));
      if (res.statusCode == 200) {
        final data = jsonDecode(res.body) as List<dynamic>;
        setState(() {
          _logs = data.map((e) => _RetrainLog.fromJson(e as Map<String, dynamic>)).toList();
          _logsLoading = false;
        });
      } else {
        setState(() {
          _logsError = 'Server returned ${res.statusCode}';
          _logsLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _triggerRetrain() async {
    ...
    try {
      final res = await http.post(Uri.parse('$_cfBaseUrl/admin/cf/retrain'));
      if (res.statusCode == 200) {
        setState(() {
        ...
        setState(() {
          ...
          _retrainBanner = 'Error ${res.statusCode}: ${res.body}';
          _retrainLoading = false;
        });
      }
    } catch (e) {
      ...
      child: Column(
        ...
        children: [
          AdminSectionHeader(
            ...
            subtitle: 'Trigger a WALS retraining job and view logs from cf_retrain_log.',
