...
class _DeleteUserDataPageState extends State<DeleteUserDataPage> {
  ...
  void _toggleGroup(List<UploadedMediaItem> group) {
    final Set<String> groupIds = group
        .map((UploadedMediaItem item) => item.id)
        .toSet();
    setState(() {
  ...
  Future<void> _deleteSelected() async {
    ...
    try {
      final UploadedMediaDeleteSummary summary = await _repository.deleteMedia(
        ids,
      );
      if (!mounted) return;
      ...
      final Set<String> failedIds = <String>{
        ...
        ...ids.where(
          (String id) =>
              !deletedIds.contains(id) && !summary.failedMediaIds.contains(id),
        ),
      };
      List<UploadedMediaItem>? reconciledItems;
      if (failedIds.isNotEmpty) {
        try {
          final List<UploadedMediaItem> currentItems = await _repository
              .loadOwnedMedia();
          final Set<String> currentIds = currentItems
              .map((UploadedMediaItem item) => item.id)
              .toSet();
      ...
      setState(() {
        if (reconciledItems != null) {
          ...
          _items.removeWhere(
            (UploadedMediaItem item) => deletedIds.contains(item.id),
          );
        }
        _selectedIds
          ..clear()
  ...
  Widget _buildMediaList() {
    ...
    final List<UploadedMediaItem> forumItems = _items
        .where(
          (UploadedMediaItem item) => item.source == UploadedMediaSource.forum,
        )
        .toList(growable: false);
    final List<UploadedMediaItem> aiItems = _items
        .where(
          (UploadedMediaItem item) => item.source == UploadedMediaSource.ai,
        )
        .toList(growable: false);
  ...
  Widget _buildGroupHeading(String label, List<UploadedMediaItem> group) {
    ...
    final bool selected = group.every(
      (UploadedMediaItem item) => _selectedIds.contains(item.id),
    );
    return Row(
      children: <Widget>[
  ...
  Widget _buildConfirmation() {
    final List<UploadedMediaItem> selectedItems = _items
        ...
        .toList(growable: false);
    final bool includesLastImageOnlyPost = selectedItems.any(
      _isLastImageOnlyPost,
    );
  ...
  bool _isLastImageOnlyPost(UploadedMediaItem item) {
    ...
    if (item.source != UploadedMediaSource.forum ||
        postId == null ||
        item.postHasText) {
      return false;
    }
    return _items
        ...
        .every(
          (UploadedMediaItem candidate) => _selectedIds.contains(candidate.id),
        );
  }
}
...
class _MediaRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final String source = item.source == UploadedMediaSource.forum
        ? 'Forum image'
        : 'AI image';
    return InkWell(
      onTap: onTap,
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          Expanded(
            child: Text(
              item.source == UploadedMediaSource.forum
                  ? 'Forum image'
                  : 'AI image',
            ),
          ),
    ...
    return const Padding(
      ...
      child: Column(
        children: <Widget>[
          ...
          SizedBox(height: 6),
          Text(
            'Images and media you upload will appear here.',
            textAlign: TextAlign.center,
          ),
        ],
      ),
