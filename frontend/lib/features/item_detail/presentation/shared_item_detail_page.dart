...
class _SharedItemDetailPageState extends State<SharedItemDetailPage> {
  ...
  Future<void> _syncFavoriteState() async {
    ...
    final favoriteType = _resolveFavoriteType();
    if (userId == null || favoriteType == null) return;

    try {
      final isFavorite = await _wishlistRepository.isFavoriteByRawId(
        type: favoriteType,
      ...
      // Keep the local fallback state when lookup cannot be resolved.
    }
  }

  FavoriteType? _resolveFavoriteType() {
    if (widget.favoriteType != null) {
      return widget.favoriteType;
    }

    final requestCategory = widget.request?.category ?? _detail.category;
